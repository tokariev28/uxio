...
**Agent 6** strips numerical scores from Agent 5 output before the synthesis call — the LLM reasons from evidence text, not from numbers it didn't generate. Agent 6 enforces a two-part structure: `reasoning` is a direct competitor-vs-input comparison (minimum 2 sentences); `exampleFromCompetitor` is the evidence anchor (exact quote, metric, or visual detail from the competitor). The Zod schema uses `competitorExample` internally and maps to `exampleFromCompetitor` on output. `overallScores` are computed programmatically from Agent 5 section data after synthesis completes, not generated by the AI. Screenshots are removed from `PageData` before the `complete` SSE event is sent (~4–12 MB payload reduction).
