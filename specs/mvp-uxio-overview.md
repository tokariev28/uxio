...
6. Sees the **InspirationGallery** — an auto-scrolling 3D carousel of 20 SaaS websites ("Websites we love") shown during analysis at the bottom of the viewport (uses `mt-auto` flex positioning to avoid layout jumps when DevTools opens).
7. When the analysis finishes, the user sees a **Results** section with:
   ...
   - A browser notification fires on completion if the tab is hidden and permission was granted. In incognito/private browsing (where permission is silently denied), the UI shows "Notifications blocked. We'll update the tab title instead." instead of silently hiding the button.
...
- Agent 1 (Multi-Signal Discovery) runs 3 parallel queries based on `ProductBrief`:
  1. `"${company} alternatives ${year}"` (weight 1.0×)
  2. `"best ${cvpKeyword} software for ${icpKeyword} ${year}"` (weight 1.2×)
  3. `"${company} vs"` (weight 1.8× — strongest signal)

Three queries provide sufficient discovery signal while keeping API costs and latency low. Results are merged and de-duplicated (by root domain) with LLM discovery results into the candidate set.
...
Automatic retry: up to 2 retries on transient errors (429, 502, 503, 504, rate limit, overloaded, timeout, econnrefused, connection reset) with 1s / 2s delays.
...
**Agent 5** uses `AGENT_PROMPTS.sectionAnalyzerBatch` (the `visionAnalyzer` key in `prompts.ts` is legacy/reference only and not in the active code path). Sections per page are capped to `MAX_SECTIONS_PER_PAGE = 8` by scrollFraction (above-the-fold priority). For competitor pages, sections are filtered to only types present on the input page — non-matching competitor sections would be discarded by Agent 6 anyway. The prompt requests exactly **1 strength and 1 weakness** per section with a mandatory evidence anchor (quoted phrase, specific number, or competitor name). Each `SectionFinding` receives a `confidence` score based on source quality: `1.0` (full visual + copy evidence), `0.7` (text-only, no screenshot — also the runtime cap enforced in code when no screenshot is available), `0.4` (inferred from sparse markdown), `0.2` (markup artifacts only, no readable copy).

**Agent 6** strips numerical scores, `confidence`, and truncates `strengths`/`weaknesses` to the first element from Agent 5 output before the synthesis call — the LLM reasons from `summary` + `evidence` text and the separately computed `scoreGaps`, not raw finding arrays. Caps to `MAX_SYNTHESIS_SECTIONS = 5` by competitive gap magnitude (tiebreak: lower scrollFraction). Uses fixed `recsPerSection = 2` — passed to the LLM as `RECOMMENDATIONS_PER_SECTION` in the user message. The `impact` field is truncated to 2 sentences in code as a safeguard. Agent 6 enforces a two-part structure: `reasoning` is a direct competitor-vs-input comparison (minimum 2 sentences); `exampleFromCompetitor` is the evidence anchor (exact quote, metric, or visual detail from the competitor). The Zod schema uses `competitorExample` internally and maps to `exampleFromCompetitor` on output. `overallScores` are computed programmatically from Agent 5 section data after synthesis completes, not generated by the AI. Screenshots are removed from `PageData` before the `complete` SSE event is sent (~4–12 MB payload reduction).
