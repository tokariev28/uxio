...
| 6 | `agent6-synthesis.ts` | Synthesis — 2 recommendations per section (top 5 sections by competitive gap) + executive summary | Gemini |
...
**Agent 0 scrape reuse**: Agent 0 scrapes the input URL with `["markdown", "screenshot"]` and stores the result in `ctx.inputPageData`. Agent 3 checks for this cached data first — if usable markdown exists, it skips re-scraping the input URL entirely, saving one Firecrawl API call (~5-10s). The screenshot URL is resolved to base64 in Agent 3 if needed. After resolving the input page (whether reused or freshly scraped), Agent 3 writes the finalised payload back to `ctx.inputPageData` — so downstream agents always see the validated markdown and the resolved base64 screenshot, not the original Agent 0 payload with a potentially expired GCS URL.

**Agent 3 scraping**: Two separate functions with different timeout budgets. `scrapePageWithRetry()` is for the input URL — 90s first pass / 120s retry. `scrapeCompetitorPage()` is for competitors — uses tighter `COMPETITOR_SCRAPE_TIMEOUT_MS = 60_000` / `COMPETITOR_RETRY_TIMEOUT_MS = 90_000` and `COMPETITOR_MAX_AGE_MS = 48h` (competitors don't redesign daily; longer cache dramatically increases hit rate). Both functions accept an optional `deadline` (absolute ms timestamp, not a duration) that compresses each pass to the remaining budget via `remaining = () => Math.max(15_000, deadline - Date.now())` — evaluated separately before each pass so both passes together stay within the budget, not each pass at the full cap. Both functions retry with `waitFor: 8000ms` if `isUsableMarkdown()` returns false and return whichever pass had more content. **Competitor success check**: `isUsableMarkdown()` is used in the `Promise.allSettled` result check — not just for retry triggering. This means Cloudflare challenge pages, cookie walls, and thin JS placeholders are treated as failures; backups are launched for them the same as for network errors. **Parallel backup competitors**: if primaries fail, all needed backups run simultaneously via `Promise.allSettled` — not sequentially. Worst-case Agent 3 time: 3 parallel primaries (60s) + 1 parallel backup round (60s) = 120s max, vs. 270s sequential worst case before this change. `scrapeDeadline` is computed at the top of `runPipeline` (`pipelineStart + PIPELINE_BUDGET_MS - 80_000`) and passed to `runScraper`. The orchestrator also gates entry to the scraping step: if `Date.now() > scrapeDeadline` before Agent 3 starts (meaning Agents 0–2 consumed >210s), the pipeline aborts immediately with a user-visible error instead of entering Agent 3 with an already-exhausted budget.
...
All three wrap calls in `withRetry()` — up to 2 retries on transient errors (429, 502, 503, 504, timeout, rate limit, overloaded, econnrefused, connection reset) with 1s/2s delays. The orchestrator also wraps each agent step in `withStepRetry()` (1 retry, immediate retry — no delay) for step-level resilience. **Exception: the scraping step has `maxRetries: 0`** — a retry would double Agent 3's runtime and risk exceeding the 290s pipeline budget. The pipeline soft budget is `PIPELINE_BUDGET_MS = 290_000`; the orchestrator checks `Date.now() - pipelineStart > PIPELINE_BUDGET_MS` before every step and aborts with a user-visible error if exhausted. The scraping step has an additional pre-entry gate: if `Date.now() > scrapeDeadline` (`pipelineStart + PIPELINE_BUDGET_MS - 80_000`), the pipeline aborts before starting Agent 3 — this catches the case where Agents 0–2 consumed >210s and the scrape budget is already gone. Fatal pipeline errors are logged via `console.error("[pipeline] Fatal error:", err)` before sending the SSE error event — check server logs for the full stack trace when debugging failures.

### SSE Streaming (`/lib/sse.ts`)

`createSSEStream()` returns `{ stream, writer }`. Use `writer.send(event)` and `writer.close()`. The writer has a `_closed` guard — if the client disconnects mid-stream, `send()` silently stops instead of throwing, and `close()` is idempotent. The API route sets `maxDuration = 300`. The orchestrator sends a heartbeat `progress` event every 20s during the scraping step to prevent CDN/proxy connections from timing out during a silent 60–90s scrape.
...
- **`scrape-quality.ts`** — `isUsableMarkdown(md)` returns `false` if content is < 300 chars or contains unusable-page signals (JS not rendered, 404/403 error pages, CAPTCHA/bot detection, cookie walls, Cloudflare challenges). Used by Agent 0 (fail fast), Agent 3 retry trigger (both passes), and Agent 3 competitor success check (ensures Cloudflare/bot-block pages aren't accepted as valid scrapes).
...
- `components/analysis/` — the three main panels (form, progress, results) plus `InspirationGallery` (auto-scrolling 3D card gallery of example sites shown during analysis). During `running` state the gallery uses `position: fixed; bottom: 0; left: 0; right: 0` — this fixes a Safari bug where `mt-auto` doesn't resolve correctly in a flex container with `min-height`. Safe here because the hero (which needs stable `min-height` to prevent gradient jump) is not visible while running. The analysis content area gets `pb-[340px]` to prevent `ProgressPanel` from being hidden behind the fixed gallery.
- `components/analysis/results/` — sub-components for the results view:
  ...
  - `InsightSlider` — horizontal slider of insight cards per section; renders competitor names as inline `<a>` links with Google favicon images via `renderReasoningText()` (style: `inline-flex`, favicon 13px, bold name, `border-bottom: 1px dashed #9ca3af`); supports keyboard navigation (ArrowLeft/ArrowRight). `ResultsPanel` reuses this exact link style for the "Compared with [competitors]" row in the results header, replacing the plain site URL link.
  ...
  - `AnalysisPDF` — `@react-pdf/renderer` document component; mirrors the results UI structure. The PDF header shows "vs [favicon]CompetitorName, ..." using `<Image>` + `<Text>` in a `flexDirection: "row"` `<View>`, replacing the plain `host` URL text
...
- `AnalysisForm.tsx` caches completed results in `localStorage` (key: `uxio:v{CACHE_VERSION}:cache:<url>`, TTL: 2 hours). On submit, a cache hit skips the pipeline entirely and shows results instantly. Bump `CACHE_VERSION` in `AnalysisForm.tsx` when `AnalysisResult` schema changes to auto-invalidate stale entries. `normalizeUrl()` only normalises the scheme (adds `https://` prefix); the URL it returns is used unchanged for validation and the API call. `toCacheKey(url)` is a separate helper that strips trailing slashes before cache reads/writes — this keeps `example.com/` and `example.com` on the same cache entry without modifying the URL that goes to the server. A `urlWarning` state detects subpage URLs (`pathname !== "/"`) and shows a non-blocking amber `.hero-url-warning` note — analysis still proceeds.
