...
**Agent 3 scraping**: Two separate functions with different timeout budgets. `scrapePageWithRetry()` is for the input URL — 90s first pass / 120s retry, accepts optional `maxTimeout` to cap both passes when the pipeline deadline is near. `scrapeCompetitorPage()` is for competitors — uses tighter `COMPETITOR_SCRAPE_TIMEOUT_MS = 60_000` / `COMPETITOR_RETRY_TIMEOUT_MS = 90_000` and `COMPETITOR_MAX_AGE_MS = 48h` (competitors don't redesign daily; longer cache dramatically increases hit rate). Both functions retry with `waitFor: 8000ms` if `isUsableMarkdown()` returns false and return whichever pass had more content. **Parallel backup competitors**: if primaries fail, all needed backups run simultaneously via `Promise.allSettled` — not sequentially. Worst-case Agent 3 time: 3 parallel primaries (60s) + 1 parallel backup round (60s) = 120s max, vs. 270s sequential worst case before this change. A `scrapeDeadline` timestamp is passed from the orchestrator; per-scrape timeouts auto-compress when budget is short.
...
All three wrap calls in `withRetry()` — up to 2 retries on transient errors (429, 502, 503, 504, timeout, rate limit, overloaded, econnrefused, connection reset) with 1s/2s delays. The orchestrator also wraps each agent step in `withStepRetry()` (1 retry, immediate retry — no delay) for step-level resilience. **Exception: the scraping step has `maxRetries: 0`** — a retry would double Agent 3's runtime and risk exceeding the 290s pipeline budget. The pipeline soft budget is `PIPELINE_BUDGET_MS = 290_000`; `withBudgetCheck()` fires between steps and aborts with a user-visible error if the budget is exhausted. Fatal pipeline errors are logged via `console.error("[pipeline] Fatal error:", err)` before sending the SSE error event — check server logs for the full stack trace when debugging failures.
