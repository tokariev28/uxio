...
Agent 1 — find competitor candidates
    ...
    The LLM leg uses TIER-1 criteria: only companies with 3+ years on market,
    thousands of verified G2/Capterra reviews, and recognizable-by-name status
    qualify. A chain-of-thought step ("What is the primary G2 category? Which
    companies are Leaders/High Performers?") anchors category selection before
    output. LLM returns 8 candidates. Cross-validates with Tavily: candidates
    appearing in both sources get a significantly higher confidence score.
    │
    ▼
Agent 2 — score and select
    Evaluates each candidate on 4 alignment axes (same buyer, same job,
    same outcome promise, same market tier). Three additional selection rules:
    DIVERSITY — at most 2 competitors from the same primary sub-category;
    INFRASTRUCTURE exclusion — generic cloud providers (AWS, Azure, GCP,
    Vertex AI, Bedrock, Azure OpenAI) are blocked as product competitors;
    TIER RULE — when matchScores are within 0.10, prefer the candidate with
    higher market presence (more search mentions, well-known brand).
    Selects top 3 as primaries, keeps positions 4–5 as backup.
    │
    ▼
Agent 3 — capture all pages (4 in parallel)
    Scrapes full-page screenshots + structured content for the input URL
    and all 3 competitors simultaneously. GCS signed screenshot URLs are
    immediately resolved to base64 while fresh (30–60 min TTL risk).
    For JavaScript-heavy pages that don't load on first request, retries
    with 8 s wait to allow client-side hydration.
...
Agent 4 — classify sections (4 in parallel)
    ...
    Deduplicates by type: only the first occurrence of each section type
    per page is retained (prevents double-counting sticky elements).
    │
    ▼
Agent 5 — analyze each page (4 in parallel)
    Active prompt: `sectionAnalyzerBatch`. One call per page: full screenshot
    + all section content together. The screenshot provides spatial context;
    the structured content enables precision per section. Evaluates against
    the 10-axis rubric. Each finding receives a confidence score:
    1.0 (full visual context), 0.7 (text-only), 0.4 (inferred from thin content).
    Evidence grounding is enforced — every strength and weakness must cite
    specific copy or a named visual element.
    │
    ▼
Agent 6 — synthesize
    Numerical scores are stripped from Agent 5 output before this call —
    the LLM reasons from evidence text, not from numbers it didn't generate.
    ...
    ▼
Result assembly
    Screenshots (base64, ~1–3 MB each) are stripped from the SSE payload
    before sending to the client — they were consumed by Agent 5 and are
    no longer needed. Saves ~4–12 MB per analysis transfer.
    │
    ▼
